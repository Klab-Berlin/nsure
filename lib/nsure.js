...
Nsure.prototype.checks.arrayOf.checkArray = function(input, options, thisAttribute, fullInput, fullRules) {
	...
	for (var i = input.length - 1; i > -1; i -= 1) {
		...
		if (
			( options.allowedTypes.length > 0 && options.allowedTypes.indexOf(lowerType) === -1 )
			|| 
			( options.allowedValues.length > 0 && options.allowedValues.indexOf(input[i]) === -1 )
		) {
			input.splice(i, 1);
		}
	}
...
Nsure.helpers.boolNsure = function(fallbackValue) {
